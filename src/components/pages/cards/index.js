...
import ArrowIcon from "../../../Icons/ArrowUpDown";
    ...
    <>
      <div className=" mx-auto  mt-0 flex max-w-[1008px] flex-col  justify-between gap-2 px-5 pt-6 md:gap-5 lg:mt-8 lg:px-0 lg:pt-0">
        ...
        <p className="regular-text text-sm leading-6 tracking-[-0.2px] text-[#5E718D] md:text-[20px] md:leading-8 md:tracking-[-0.3px]">
          ...
          <span className="bold-text">up to 9.41% p.a.</span> FDs are regulated
          by RBI.
        </p>
      </div>
      {!discoverDatError && (
            ...
            >
              <div className="flex items-center justify-between">
                <InvestSectionHeaderWithIcon
                  headerText={`${capitalizeWords(type)} FD${fdData?.length > 1 ? "s" : ""}`}
                />

                {fdData?.length > 0 && (
                  <span className="flex items-center gap-2"> <span className="medium-text text-xs text-[#455468] md:text-sm">
                  ...
                  <button
                    ...
                    className={`flex h-[38px] min-w-[38px] cursor-pointer items-center justify-center gap-1 rounded-md border-2  px-1 transition-all duration-200 ease-in-out active:scale-[0.97]  md:h-[42px] md:min-w-[42px] md:px-2 ${order!=="asc" && " border-custom-green"}`}
                  >
                 
                    <ArrowIcon color={order!=="asc" ? "#21B546" :"#455468" } /> 
                  </button>
                  </span>
                )}
              </div>
